/**
 * GLSL parser for codemirror
 * Based on Java parser by Patrick Wied
 * @version 2011-03-27
 */

var GLSLParser = Editor.Parser = (function() {
  // Token types that can be considered to be atoms.
  var atomicTypes = {"atom": true, "number": true, "string": true, "regexp": true};
  // Constructor for the lexical context objects.
  function GLSLLexical(indented, column, type, align, prev, info) {
  ...
  // glsl indentation rules.
  function indentGLSL(lexical) {
  ...
  function parseGLSL(input, basecolumn) {
    // Wrap the input in a token stream
    var tokens = tokenizeGLSL(input);
    ...
    var lexical = new GLSLLexical(basecolumn || 0, 0, "block", false);
    ...
    function next(){
      ...
      if (token.content == "\n"){
        ...
        // them.
        token.indentation = indentGLSL(lexical);

      }
      // No more processing for meaningless tokens.
      if (token.type == "whitespace" || token.type == "comment" || token.type == "define")
        return token;
    ...
    function copy(){
      ...
      return function copyParser(input){
        ...
        column = indented = 0;
        tokens = tokenizeGLSL(input, _tokenState);
        return parser;
      };
    }
    ...
    function pushlex(type, info) {
      var result = function(){
        lexical = new GLSLLexical(indented, column, type, null, lexical, info)
      };
      result.lex = true;
      return result;
    }
